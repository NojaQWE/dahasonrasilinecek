...
if (1 > 0 && limit.has(author.id) && limit.get(author.id) == 1) return channel.send("Saatlik kayıtsız etiketleme sınırına ulaştınız!").catch(err => console.log(err), client.tick(message)).then(m => m.delete({timeout: 10000}));
        let role =  guild.roles.cache.find(rol => rol.id === config.registration.shinobi)
        channel.send(`<@&${config.registration.shinobi}> merhaba, sizi teyit kanallarına bekliyoruz.`)
    if (1 > 0) {
