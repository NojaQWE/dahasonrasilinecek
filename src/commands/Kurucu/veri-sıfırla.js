...
msg.edit({
  ...
  if(button.customId === "teyit_sıfırla") {
    await button.deferUpdate();
    let gennin = db.delete(`gennin_${member.id}`) || [];
    let chunnin = db.delete(`chunnin_${member.id}`) || [];
    let jounnin = db.delete(`jounnin_${member.id}`) || [];
    let toplam = db.delete(`toplam_${member.id}`) || [];
    const teyit = new MessageEmbed()
    .setDescription(`${member.toString()} kullanıcısının kademe geçmişi ${message.author} tarafından başarıyla temizlendi.`) 
